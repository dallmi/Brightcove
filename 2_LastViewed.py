...
def main():
    ...
    for name, cfg in ACCOUNTS.items():
        ...
        # 3) fetch rows via API call if they are not part of time window that was marked as done in checkpoint file
        i = 0
        while i < len(windows):
            for frm, to in tqdm(windows, desc=f"Fetching slices for {name}"):
                key = f"{frm}_{to}" 
                ...
                for attempt in range(1, max_retries + 1):
                    ...
                    i += 1
                    continue
                # only advance index if window succeeded or was skipped, not if subdivided
                if key in windows_done:
                    i += 1
